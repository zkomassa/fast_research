...
for f in glob.glob("*.fft"):
	print f
	subprocess.call(['accelsearch', '-zmax', '0', f], stdout=FNULL, stderr=subprocess.STDOUT)
...
cands_regex = re.findall("ACCEL_0:(\d+)\s+(\d+\.?\d*)\s+\d+\.?\d*\s+\d+\.?\d*\s+\d+\.?\d*\s+\d+\.?\d*\s+\d+\.?\d*\s+(\d+\.?\d*)\s+", cands_txt)
