...
#A script to process single data files through the PRESTO pipeline.

#command line arguments are in the format: ddscript_[n].bash directory filename
...
ln -s $directory/$filename.fits ./

echo "************************************************************"

#run rfifind:
echo "Running rfifind..."
rfifind -o ./$filename -time 2 ./$filename.fits >> /dev/null
...
#get variables needed for DDplan.py from readfile:
 ...
 #number of channels
 num_chan=`readfile $filename.fits | grep "Number of channels" | awk '{print $5}'`
 #sample time
 smp_time=`readfile $filename.fits | grep "Sample time" | awk '{print $5}'`
...
do
	...
	prepsubband -nsub $nsub -lodm $ldm -dmstep $dms -numdms $ndm -numout $numout1 -downsamp $ds -mask ${filename}_rfifind.mask -o $filename $filename.fits >> /dev/null
done
...
do
	...
	#get raw data plots
	string1="prepfold -mask ${filename}_rfifind.mask -accelcand $candnum -nsub $nsub -dm ${dmsarr[$i]} -accelfile $accelfilename.cand ${filename}.fits -o ${filename}_${dmsarr[$i]} -nosearch -noxwin" 
	echo $string1
	$string1
done
